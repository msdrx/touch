...
def orchestrator_root(root=None, env=None) -> str:
    """Where the task folders live: arg > `$TOUCH_LEGACY_ROOT` > `<repo>/…`.

    The repo-relative default is derived from this file's location, the same way
    `store.state_root` derives `.touch/`: a daemon started from anywhere reads
    the same folders, and there is no cwd-shaped way to point the adapter at
    another checkout's history (GD-12's wrong-target invariant).
    """
    if root:
    ...
    return os.path.join(repo, TASK_ROOT)
