...
const window = {
    TOUCH_TOKEN: "",
    location: {
        href: "http://127.0.0.1:8930/", search: "", hash: "#run/%zz", protocol: "http:",
...
(async () => {
    console.log("sockets opened at boot = " + sockets.length);
    console.log("log rows = " + byId.log.children.length +
                " connText = " + byId.connText.textContent +
                " detailHead = " + JSON.stringify(byId.detailHead.textContent));
    console.log("notice = " + JSON.stringify(byId.notice.textContent));
    process.exit(0);
